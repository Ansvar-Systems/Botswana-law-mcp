/**
 * get_botswanan_implementations — Find Botswanan statutes implementing a specific EU directive/regulation
...
export interface GetBotswananImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface BotswananImplementationResult {
  document_id: string;
...
export async function getBotswananImplementations(
  db: InstanceType<typeof Database>,
  input: GetBotswananImplementationsInput,
): Promise<ToolResponse<BotswananImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as BotswananImplementationResult[];
