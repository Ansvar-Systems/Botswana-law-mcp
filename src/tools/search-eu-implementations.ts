/**
 * search_eu_implementations — Search EU directives/regulations that have Botswanan implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_botswanan_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  botswanan_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as botswanan_statute_count
  ...
  if (input.has_botswanan_implementation) {
    sql += ' HAVING botswanan_statute_count > 0';
  }
