#!/usr/bin/env tsx
/**
 * Drift detection for Botswana Law MCP.
 *
 * Checks if upstream Botswana Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Botswana Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'botswana-law-mcp/1.0 drift-detect' },
      });
