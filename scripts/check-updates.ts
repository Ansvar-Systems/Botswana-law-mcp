#!/usr/bin/env tsx
/**
 * Botswanan Law MCP — Data Freshness Checker
 *
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/botswana-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Botswanan Law MCP — Data Freshness Check');
